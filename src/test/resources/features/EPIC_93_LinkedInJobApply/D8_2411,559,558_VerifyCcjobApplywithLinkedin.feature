#Story:D8-2411,559,558,561
@ApplyCCjobwithLinkedin
Feature: User will apply CC enabled job via LinkedIn profile 
Scenario: Anonymous user navigates to Job Details Page and Process the CC enabled job application with LinkedIn profile 
	...
	And I verify "Job Role Editable" on ApplyWithLinkedIn Page 
	And I Click on "Next" button on ApplyWithLinkedIn Page 
	Then I verify "Country Name" field are Prepopulated on CC_Form 
	Then I verify "City Name" field are Prepopulated on CC_Form 
	Then I verify "Nationality Name" field are Prepopulated on CC_Form 
	And I Click on "CCJobNext" button on ApplywithyourCV screen 
	Then I verify "Industry Name" field are Prepopulated on CC_Form 
	Then I verify "SubIndustry Name" field are Prepopulated on CC_Form 
	Then I verify "Sector Name" field are Prepopulated on CC_Form 
	Then I verify "Salary" field are Prepopulated on CC_Form 
	Then I verify "Currency Name" field are Prepopulated on CC_Form 
	Then I verify "Package" field are Prepopulated on CC_Form 
	
Scenario: Authenticate user navigates to Job Details Page and Process the CC enabled job application with LinkedIn profile 
